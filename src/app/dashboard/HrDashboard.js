...
export default function HrDashboard({ user, userProfile, onLogout }) {
  ...
  const tabs = ["overview", "analytics", "departments"];
...
function OverviewTab({
  ...
  const generateTrendsFromTasks = (tasks) => {
    ...
    return Object.values(monthlyTrends).sort((a, b) =>
  ...
  const validTrendsData =
    ...
    trends.length > 0 &&
    trends.some(
      (t) => t.period && t.completed !== undefined && t.total !== undefined
    );

  // Use this for clarity in the component
  const effectiveTrends = validTrendsData
    ? trends
    : [
    ...
    datasets: [
      {
        ...
        data: effectiveTrends.map((t) =>
          t && typeof t.completed === "number" ? t.completed : 0
        ),
        borderColor: "#10B981",
      ...
      },
      {
        label: "Total Tasks",
        data: effectiveTrends.map((t) =>
          t && typeof t.total === "number" ? t.total : 0
        ),
        borderColor: "#3B82F6",
...
function AnalyticsTab({ performanceRankings, trends, departments }) {
  ...
  const performanceDistribution = {
    excellent: performanceRankings.filter((emp) => emp.performanceScore >= 90)
      .length,
    good: performanceRankings.filter(
      (emp) => emp.performanceScore >= 70 && emp.performanceScore < 90
    ).length,
    average: performanceRankings.filter(
      (emp) => emp.performanceScore >= 50 && emp.performanceScore < 70
    ).length,
    needsHelp: performanceRankings.filter((emp) => emp.performanceScore < 50)
      .length,
  };
              ...
              {performanceRankings.map((emp) => {
                ...
                return (
                  <tr key={emp.id}>
                    ...
                    <td
                      className={`px-6 py-4 whitespace-nowrap text-center text-sm font-medium ${status.color}`}
                    >
                      {status.text}
                    </td>
                  </tr>
    ...
    <div className="bg-white p-4 rounded-lg shadow mb-6">
      <div className="grid grid-cols-4 gap-4">
        ...
        <select
          ...
          onChange={(e) =>
            setFilters({ ...filters, performanceLevel: e.target.value })
          }
          className="border rounded p-2"
        >
