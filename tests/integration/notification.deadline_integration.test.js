...
describeIf(hasTestEnv)('Notification Deadline Integration Tests', () => {
  ...
  describe('Real-time notification delivery (immediate vs scheduled)', () => {
      ...
      const tasksData = [
        {
          ...
          due_date: new Date(singaporeTime.getTime() + 1 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
          days: 1
        },
        {
          title: 'Task Due in 3 Days',
          due_date: new Date(singaporeTime.getTime() + 3 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
          days: 3
        },
        {
          title: 'Task Due in 7 Days',
          due_date: new Date(singaporeTime.getTime() + 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
          days: 7
        }
      ];
    ...
    it('should create notifications for overdue tasks immediately', async () => {
      ...
      expect(missedNotifications.length).toBe(3); // Owner + 2 collaborators
